...
import {
    NAME,
    TYPE,
    CREATE_PSWRD_RESET_BLOCK_INDEX_PREFIX,
    LABEL,
    PSWD_RESET_URL,
    CREATE_PSWRD_RESET_DEF,
    REGEXES,
    ERROR_MSG,
    CREATE_PSWRD_RESET_ERROR_MSG_PREFIX,
    CONFIRM,
} from "../../constants";
...
class CreatePasswordResetForm extends React.Component {
    ...
    handleClickSubmit() {
        ...
        postCallApi(PSWD_RESET_URL, formValues, false)
            ...
            .then((jsonData) => {
                if (failure) {
                    this.setState(
                        Object.assign(
                            ...
                            { status: "input" },
                            {
                                [jsonData.wrongField]: Object.assign(
                                    {},
                                    this.state[jsonData.wrongField],
                                    {
                                        valid: false,
                ...
                } else this.setState({ status: "sent" });
            })
            .catch((error) => {
    ...
    validateField(name, value) {
        ...
        const newField = {
            ...
            errorMsg: valid
                ? ""
                : CREATE_PSWRD_RESET_DEF.find((elem) => elem[NAME] === name)[ERROR_MSG],
        };
        ...
        return (
            ...
            >
                ...
                <Message error>
                    <List bulleted>
                        {Object.keys(this.state).map((key) => {
                            ...
                            if (errorMsg)
                                return (
                                    <List.Item key={CREATE_PSWRD_RESET_ERROR_MSG_PREFIX + key}>
                                        {errorMsg}
                                    </List.Item>
                                );
                        })}
                ...
                <Message info icon hidden={this.state.status !== "loading"}>
                    ...
                    <Message.Content>Envoi du mail en cours...</Message.Content>
                </Message>
            </Form>
        );
    }
